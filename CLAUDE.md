...
0. **Shared vocabulary** (`shared/amenities.ts`) — Amenity types, `PSEUDO_AMENITIES`, `normalizeElement` and `overpassQuery`, imported by **both** the browser bundle and the Functions. ⚠️ Keep it free of React/DOM/localforage/Node — `functions/tsconfig.json` compiles it against `@cloudflare/workers-types` with no DOM lib, which is what enforces this.
   - **`PSEUDO_AMENITIES`** is the single table for amenities OSM files under a key other than `amenity` (`leisure=playground`, `leisure=picnic_table`, `tourism=picnic_site`, `highway=elevator`). It drives **both** the query branches and `normalizeElement`, so the two can't drift. Add new ones there, not in two places. ⚠️ It must be declared before `editableAmenities`, which reads it at module load.
     - **Writability is a per-entry flag (`writable`), not a property of the table.** The write path serializes the real pair via `toOsmTags` (`amenity=playground` means nothing in OSM; `leisure=playground` is the tag), so a pseudo-amenity is editable as soon as it declares which pair to write. `playground` does (44% of Italian playgrounds are plain nodes — Overpass 2026-08-10: 10,457 nodes vs 13,343 ways); `elevator` would only need the flag (86% are nodes, taginfo 2026-08-09).
     - ⚠️ **`picnic` cannot have the flag, and that is structural**: two tags (`leisure=picnic_table`, `tourism=picnic_site`) fold into one amenity, so there is no inverse — nothing in `amenity: "picnic"` says which the user meant. `osmTagFor` returns undefined unless *exactly one* entry is writable; don't "fix" it by picking the first.
     - ⚠️ **A writable pseudo-amenity can displace a foreign `amenity` tag, and writing the app's model straight back would delete it from OSM.** `leisure=playground` + `amenity=traffic_park` folds to `amenity: "playground"` and the foreign value is gone from the model. So `tagsForOsm` takes what OSM currently holds (already fetched for the version) and restores `amenity` from it: the app can't represent that tag, so it doesn't get a vote on it. Rare, not hypothetical — 12 of Italy's 10,457 playground nodes.
...
Authenticated users can create, update, and delete OSM nodes. **Write path is node-only**: `isEditable` gates it to plain nodes with an editable amenity, and `editableAmenities` gates the add menu. Anything mapped as a way/relation is read-only whatever its amenity — most playgrounds are areas, and the ones that aren't can be edited like any other node. Picnic areas and elevators stay read-only, for the reasons in `PSEUDO_AMENITIES` above. `osm-auth` handles OAuth2 in singlepage mode (redirect-based, no popup); the OAuth app is a public client (no client_secret).
