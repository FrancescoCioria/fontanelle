...
- **Marker components** (`DrinkingWaterMarker.tsx`, etc.) — SVG icons. Colour indicates **usability**: white=open, grey=closed/not-public (`getAmenityColor` → `statusColor`)
  - ⚠️ **A fee is a badge, not a colour** (`FeeBadge.tsx`, 2026-08-11, following the same fix in `viaggi`). Gold used to be a third value of `getAmenityColor`, which made one string carry two independent facts and could only ever show one: a toilet that is *both* paid and closed came out plain grey, price dropped silently. The coin is the gold that left the fill (#eab308), sits top-right (the changing-table badge owns bottom-right, and they co-occur), and composes with any fill. `hasFee` is the predicate; `fee` is the marker prop.
